...
class ProductController extends Controller
{
    ...
    public function assignMediaToAttributes(Request $request, $id)
    {
        ...
        foreach ($filteredAttributes as $attribute) {
            $product->attributeValues()->updateExistingPivot($attribute['attribute_value_id'], ['media_id' => $attribute['media_id']]);
        }

        // Provide a success response
        $notify[] = ['success', 'Media assigned successfully to attribute values'];
        return redirect()
            ->to(route('admin.products.edit', $product->id) . '#media-content')
            ->withNotify($notify);
    }
}
