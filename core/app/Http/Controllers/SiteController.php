...
class SiteController extends Controller
{
    ...
    //     // Paginate categories (4 per page)
    //     $categories = Category::featured()->orderBy('name', 'asc')->paginate(4);

    //     // Paginate products (9 per page)
    //     $products = Product::published()
    //         ->whereHas('categories')
    //         ->with(['categories', 'displayImage'])
    //         ->orderBy('name', 'asc')
    //         ->paginate(9);

    //     // Latest Products
    //     $products_latest = Product::published()
    //         ->with(['categories', 'displayImage'])
    ...
    //     // Check if request is AJAX (for "Load More")
    //     if ($request->ajax()) {
    //         if ($request->has('categories')) {
    //             return view('partials.categories', compact('categories'))->render();
    //         }
    ...
    //     return view('Template::home', compact('pageTitle', 'sections', 'seoContents', 'seoImage', 'products', 'products_latest', 'categories', 'brands'));
    // }
    public function index(Request $request)
    {
        $pageTitle = 'Home';
        ...
        // Fetch specific categories by name (or by slug if necessary)
        $categories = Category::whereIn('name', ['Mens', 'Women', 'Extensions', 'Hair System Supplies'])
            ->orderByRaw("FIELD(name, 'Mens', 'Women', 'Extensions', 'Hair System Supplies')")
            ->get();
    ...
    /**
     * Recursively gather subcategory IDs.
     */
    ...
    public function about()
    {
        ...
        return view('Template::about', compact('pageTitle'));
    }
    ...
    public function mens_hair_system()
    {
        ...
        return view('Template::mens_hair_system', compact('pageTitle', 'sections'));
    }
}
