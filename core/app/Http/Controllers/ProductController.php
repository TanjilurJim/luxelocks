...
class ProductController extends Controller
{
    ...
    {   $products = Product::all();
        ...
        return $this->getProducts('shopPageProducts', $products);
    }
    ...
    public function show($id)
    {
        ...
        return view('Template::north_hair.product_show', compact('product'));
    }
    ...
    //     return view('Template::product_show', $data);
    // }
    ...
    public function productDetails($slug)
{
    ...
    $baseSizes = $product->attributeValues()
    ...
    // Ensure `relatedProducts` is passed to avoid errors
    $relatedProducts = $otherProducts ?? collect();

    $data = compact('product', 'images', 'quickView', 'relatedProducts','baseSizes');
...
public function cart(Request $request)
{
    ...
    $baseSize  = $request->input('base_size'); // The user-chosen size

    // Add to cart logic
    // Example: Store in session or database
    $cart = session()->get('cart', []);

    $key = $variantId ? "product_{$productId}_variant_{$variantId}" : "product_{$productId}";
    ...
    if (isset($cart[$key])) {
        ...
        $cart[$key] = [
            ...
            'quantity' => $quantity,
            'base_size'  => $baseSize,   // ✅ store the user-chosen size here
        ];
    }
    ...
    private function productDetailsRelations()
    {
        ...
        return [
            ...
            'productVariants' => function ($productVariant) {
                return $productVariant->published()->with('displayImage');
            },
