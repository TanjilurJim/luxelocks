...
{
    // 1. If the requested length is 0, just return 0.
    if ($length == 0) return 0;

    // 2. Calculate the smallest number with the given number of digits.
    //    For example, if $length = 3, pow(10, 2) = 100, which is the smallest 3-digit number.
    $min = pow(10, $length - 1);

    // 3. Calculate the largest number with that many digits.
    //    - ($min - 1) drops it by 1, e.g. 100 - 1 = 99.
    //    - appending '9' effectively puts a 9 at the end, e.g. '99' . '9' = '999'.
    //    - (int) '999' converts that string back to integer 999.
    //    - So if $min = 100, $max becomes 999, which is the largest 3-digit number.
    $max = (int) ($min - 1) . '9';

    // 4. Generate a random integer between $min and $max (inclusive).
    //    This ensures the result has exactly the requested number of digits.
    return random_int($min, $max);
}

function getNumber($length = 8)
{
    // 1. Define the characters (digits) to choose from.
    $characters = '1234567890';

    // 2. Get the length of the $characters string.
    //    For "1234567890", this is 10.
    $charactersLength = strlen($characters);

    // 3. Initialize an empty string to build the random string.
    $randomString = '';

    // 4. Loop as many times as $length (default is 8).
    //    Each iteration will add one random digit to $randomString.
    for ($i = 0; $i < $length; $i++) {
        // Pick a random index between 0 and (length of characters - 1).
        $randomIndex = rand(0, $charactersLength - 1);

        // Append the character at that index to $randomString.
        $randomString .= $characters[$randomIndex];
    }

    // 5. Return the random numeric string.
    return $randomString;
}
...
function getContent($dataKeys, $singleQuery = false, $limit = null, $orderById = false)
{
    ...
    \Log::info('Active Template Name: ' . $templateName);
...
function getMultipleContent($dataKeys, $singleQuery = false, $limit = null, $orderById = false)
{

    $templateName = activeTemplateName();
    \Log::info('Active Template Name: ' . $templateName);
    if ($singleQuery) {
        $content = Frontend::where('tempname', $templateName)->where('data_keys', $dataKeys)->orderBy('id', 'desc')->get();
    } else {
        $article = Frontend::where('tempname', $templateName);
        $article->when($limit != null, function ($q) use ($limit) {
            return $q->limit($limit);
...
function getSectionData($key, $isOnlyData = false, $isFrontend = false)
{
    ...
    try {
        $modelIndex = $key; // banner_4
        $parts = explode('_', $modelIndex); // banner, 4
        $modelIndex = $parts[0] . '_'; // banner_
