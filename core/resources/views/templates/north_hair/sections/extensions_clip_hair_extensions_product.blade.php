@php
    // Get the content for the Clip-In Hair Extensions Section
    $content = getContent('extensions_clip_hair_extensions_product.content', true);
...
@if ($data_values)
    ...
    <section>
        <div class="max-w-screen-xl mx-auto px-4 mt-2">
            <h2 class="md:text-[30px] text-[26px] font-bold">{{ $data_values->title ?? 'Clip-In Hair Extensions' }}</h2>
                ...
                <div class="group relative">
                    <a href="{{ route('product.detail', $product->slug) }}">
                        ...
                        <p class="text-[18px] mb-3 mx-5 mt-3">
                            <span class="font-bold">{{ $product->sku ?? 'SKU:' }} :</span> <strong>
                                {{ $product->name }}</strong>
                        </p>
                    </a>
...
<!-- JavaScript for Expand/Collapse Feature -->
<script>
