@php
    $content = getContent('women_wigs_and_toppers_product.content', true);
                ...
                <div class="product-item group relative">
                    <a href="{{ url('/product/' . $product->slug) }}"> <!-- Changed to direct URL -->
                        ...
                        <p class="text-[18px] mb-3 mx-5 mt-3">
                            <span class="font-bold">{{ $product->sku ?? 'SKU:' }} :</span> <strong>{{ $product->name }}</strong>
                        </p>
                    </a>
...
<script>
    ...
    loadMoreBtn?.addEventListener('click', function () {
        ...
        fetch(`?page=${nextPage}`)
            ...
            .then(data => {
                ...
                if (!hasMorePages) {
                    loadMoreBtn.classList.add('hidden'); // Hide Load More button if no more pages
                } else {
    ...
    document.querySelectorAll(".show-more-btn").forEach(button => {
        button.addEventListener("click", function () {
            ...
            let moreImagesContainer = document.querySelector(`.extra-thumbnails[data-product-id='${productId}']`);
