    ...
    menuItems.forEach(item => {
        item.addEventListener("click", function () {
            ...
            submenus.forEach(sub => {
                if (sub !== submenu) {
                    sub.classList.remove("max-h-[1000px]", "opacity-100", "block");
                    sub.classList.add("max-h-0", "opacity-0", "hidden");
                }
            });

            // Toggle the clicked submenu
            if (submenu.classList.contains("max-h-0")) {
                submenu.classList.remove("max-h-0", "opacity-0", "hidden");
                submenu.classList.add("max-h-[1000px]", "opacity-100", "block");
            } else {
                submenu.classList.add("max-h-0", "opacity-0", "hidden");
                submenu.classList.remove("max-h-[1000px]", "opacity-100", "block");
            }
        });
    });
});
